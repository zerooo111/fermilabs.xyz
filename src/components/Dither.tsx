...
void main() {
  ...
  // Create 3 layered waves with different frequencies and offsets
  ...
  // Multi-color gradient: dark green -> medium green -> light green -> yellow-green
  vec3 col;
  if (f < 0.33) {
...
interface WaveUniforms {
  ...
  darkColor: THREE.Uniform<THREE.Color>;
  midColor: THREE.Uniform<THREE.Color>;
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<WaveUniforms>({
    ...
    darkColor: new THREE.Uniform(new THREE.Color(...darkColor)),
    midColor: new THREE.Uniform(new THREE.Color(...midColor)),
  ...
  const prevDarkColor = useRef([...darkColor]);
  const prevMidColor = useRef([...midColor]);
  useFrame(({ clock }) => {
    ...
    if (!prevDarkColor.current.every((v, i) => v === darkColor[i])) {
      u.darkColor.value.set(...darkColor);
      prevDarkColor.current = [...darkColor];
    }
    if (!prevMidColor.current.every((v, i) => v === midColor[i])) {
      u.midColor.value.set(...midColor);
      prevMidColor.current = [...midColor];
    }
